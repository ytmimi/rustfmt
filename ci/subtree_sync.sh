...
function clone_rustlang_rust() {
    ...
    echo "Cloning $RUST_LANG_RUST_GIT_URL into $CLONE_DIR"
    # Do we need the entire git history? Would it suffice to just get the history from the last full subtree sync?
    git clone -q --branch master --single-branch $RUST_LANG_RUST_GIT_URL $CLONE_DIR
    cd $CLONE_DIR
}
...
function try_create_subtree_push_merge_commit() {
    ...
    echo "Trying create merge commit between $NEW_BRANCH_NAME and $REMOTE_REF"
...
function create_subtree_push_pull_request() {
    ...
    local GIT_URL="https://github.com/$GITHUB_REPOSITORY.git"
...
[Diff-Check]: $REPOSITORY_URL/actions/workflows/check_diff.yml
"
    fi
...
"
    ...
    local RUSTFMT_REMOTE_HEAD=$(git remote show origin | grep 'HEAD branch' | cut -d' ' -f5)
